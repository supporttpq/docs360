...
**Extras Setup → Discounts/Supplements**

### Interface overview

<div data-with-frame="true"><figure><img src="../.gitbook/assets/Disc overvier.png" alt="Discounts/Supplements overview page"><figcaption></figcaption></figure></div>

The page has two main areas:

1. Filter and search controls.
2. Discounts/Supplements list.

#### Filters

<div data-with-frame="true"><figure><img src="../.gitbook/assets/disc filter section.png" alt="Discounts/Supplements filter controls"><figcaption></figcaption></figure></div>
...
{% hint style="info" %}
Click **+ More filters** to expand the full filter panel.
{% endhint %}

#### List overview

<div data-with-frame="true"><figure><img src="../.gitbook/assets/disc list.png" alt="Discounts/Supplements list overview"><figcaption></figcaption></figure></div>

**List columns**

Each row in the list represents one discount or supplement record.

<table><thead><tr><th width="222.75">Field Name</th><th width="254.5">Description</th><th>Notes / System Impact</th></tr></thead><tbody><tr><td>CODE</td><td>Unique identifier for the discount/supplement</td><td>Codes are system-wide references used in booking rules and exports. Click the code to open the record for editing.</td></tr><tr><td>NAME</td><td>Display name of the discount/supplement</td><td>This name is displayed in internal tools and reports.</td></tr><tr><td>DISCOUNT/SUPPLEMENT</td><td>Defines if entry is a discount or supplement</td><td>Indicates whether the record <strong>reduces</strong> the price (<em>Discount</em>) or <strong>adds</strong> to it (<em>Supplement</em>).</td></tr><tr><td>GENERAL/SPECIFICATE</td><td>Defines scope of applicability</td><td><p>Controls how broadly the record is applied:</p><ul><li><strong>General</strong> — applies across all applicable bookings without further filtering.</li><li><strong>Specificate</strong> — applies only to bookings that match specific criteria defined in the record (e.g. a particular hotel, room type, or departure period).</li></ul></td></tr><tr><td>FIXED/MANUAL</td><td>Defines pricing behavior</td><td><p>Determines how the price adjustment is triggered:</p><ul><li><strong>Fixed</strong> — automatically applied when the booking matches the defined conditions.</li><li><strong>Manual</strong> — must be added to a booking deliberately by an agent.</li></ul></td></tr><tr><td>PRICE</td><td>Monetary value</td><td>Applied to booking calculations</td></tr><tr><td>FAMILY DISCOUNT</td><td>Indicates family discount support</td><td>Indicates whether this record qualifies as a <strong>family discount</strong> (icon: ✅ yes / ❌ no). Family discounts may be subject to specific eligibility rules defined in system settings.</td></tr><tr><td>GENERIC SERVICES</td><td>Indicates link to generic services</td><td>Indicates whether the record applies to <strong>generic services</strong> (icon: ✅ yes / ❌ no). Generic services are add-ons or inclusions not tied to a specific product line.</td></tr><tr><td>ONE-WAY</td><td>Indicates one-way applicability</td><td>Used for transport pricing</td></tr></tbody></table>
...
1. In **Discounts/Supplements**, click **Create**.
2. Configure the name, type, pricing behavior, and applicability.
3. Define booking and departure periods.
4. Assign destinations, hotels, and transports if required.
5. Save the configuration.

#### Filter existing Discounts/Supplements

1. In the filter panel, enter filter criteria.
2. Click **Display**.
3. Review matching configurations.

#### Delete a Discount/Supplement

1. In the list, locate the record.
2. At the end of the row, click the trash icon.
3. Confirm deletion.
...
**Result:** Customers booking early receive an automatic reduction.
...
**Result:** Staff apply an additional transport fee manually.
...
**Result:** Special family pricing becomes available.
